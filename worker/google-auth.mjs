...
export function parseServiceAccount(text) {
  ...
  catch { throw new Error(tr('GOOGLE_SERVICE_ACCOUNT_JSON is missing or is not the complete JSON key. Paste the whole downloaded key file into the repository secret.')); }
  if (credentials?.type !== 'service_account' || typeof credentials.client_email !== 'string' || !credentials.client_email.endsWith('.iam.gserviceaccount.com')) {
    throw new Error(tr('GOOGLE_SERVICE_ACCOUNT_JSON is not a service-account key from your own Google Cloud project.'));
  }
  try {
    const key = createPrivateKey(credentials.private_key);
    if (key.asymmetricKeyType !== 'rsa') throw new Error();
  } catch {
    throw new Error(tr('The service-account private key is missing or invalid. Create a new JSON key and replace the secret.'));
  }
  return credentials;
}
...
export function tokenProvider(credentials, scopeName, { fetchFn = fetch, now = Date.now } = {}) {
  ...
  return async function getToken() {
    ...
    try {
      ...
      throw new Error(tr('Google sign-in: connection failed or timed out.'));
    }
    if (!response.ok) {
      const hint = response.status === 400 ? tr('Check the service-account key and the runner clock.') : response.status === 401 ? tr('The credential was rejected. The key may have been deleted or revoked.') : tr('Google could not sign the worker in.');
      throw new Error(tr('Google sign-in: HTTP {0}. {1}', response.status, hint));
    }
    const data = await response.json().catch(() => ({}));
    if (typeof data.access_token !== 'string' || !data.access_token) throw new Error(tr('Google sign-in returned no access token.'));
