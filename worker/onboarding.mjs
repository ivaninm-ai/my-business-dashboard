...
function invalidProposal(parsed) {
  const shape = parsed && typeof parsed === 'object' && !Array.isArray(parsed)
    ...
    : Array.isArray(parsed) ? tr('a list') : typeof parsed;
  return new Error(tr('Gemini returned an invalid proposal (received {0}). Retry source preparation.', shape || 'nothing'));
}
...
export async function prepareProposal(request, sources, options) {
  if (request.aiDataMode !== 'paid' && request.profile?.synthetic !== true) throw new Error(tr('For real private records, select a billing-enabled Gemini project in Business setup. Free mode accepts synthetic practice data only.'));
  ...
  if (request.source.kind === 'google_sheet') {
    ...
    catch { throw new Error(tr('Cannot read this Google Sheet. Share the native Google Sheet with your service-account email as Viewer, then retry.')); }
    if (info.sheets.length > 12) throw new Error(tr('This workbook has more than 12 tabs. Use a smaller workbook.'));
  ...
  if (input.text) {
    if (!Array.isArray(parsed.document_tables) || !parsed.document_tables.length) throw new Error(tr('No supported records found in this document. Use a text-based table or a spreadsheet export.'));
  ...
  if (!selections.length) throw new Error(tr('No supported customer, order, received-payment or stock table was found. Use a detailed record export rather than a summary report.'));
  for (const s of selections) if (!Array.isArray(s.fields)) throw new Error(tr('Gemini returned an incomplete mapping. Retry.'));
...
export async function runOnboarding({ credentials, workspaceId, apiKey, model = DEFAULT_MODEL, fetchFn, clientFactory }) {
  ...
  useWorkerLocale(request.profile?.locale);
