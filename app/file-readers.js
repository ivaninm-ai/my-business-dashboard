...
export async function extractFile(file) {
  if (!file || file.size > SETUP_LIMITS.fileBytes) throw new Error('Choose a file no larger than 10 MB.');
  const extension = file.name.split('.').pop().toLowerCase();
  let input;
  if (extension === 'csv') {
    const parsed = Papa.parse(await file.text(), { skipEmptyLines: 'greedy' });
    if (parsed.errors.length) throw new Error('Could not read this CSV: ' + parsed.errors[0].message);
    input = { tables: [{ name: file.name, rows: parsed.data }] };
  } else if (extension === 'xlsx') {
    const sheets = await readXlsxFile(file);
    if (sheets.length > SETUP_LIMITS.tables) throw new Error('Use a workbook with at most 12 sheets.');
    const tables = [];
    ...
    try {
      if (document.numPages > 50) throw new Error('Use a PDF with at most 50 pages.');
      const pages = [];
      for (let n = 1; n <= document.numPages; n++) {
        const page = await document.getPage(n);
        const content = await page.getTextContent();
        const text = content.items.map(x => x.str + (x.hasEOL ? '\n' : ' ')).join('');
        if (!text.trim()) throw new Error(`PDF page ${n} has no readable text. Scanned pages need OCR; use a spreadsheet or text-based PDF.`);
        pages.push(`[Page ${n}]\n${text}`);
      }
      input = { tables: [], text: pages.join('\n\n') };
    } finally { await task.destroy(); }
  } else throw new Error('Choose XLSX, CSV, a text-based PDF, or DOCX. Save older XLS files as XLSX first.');
  if (!input.tables.length && !input.text?.trim()) throw new Error('The file has no readable records.');
