...
export function createSheetsClient({ apiBase = DEFAULT_API_BASE, getToken, fetchFn = globalThis.fetch, timeoutMs = 30000 }) {
  ...
  async function call(stage, method, pathAndQuery, body) {
    ...
    try {
      ...
      throw new SheetsError(`${stage}: could not reach Google Sheets (network error or timeout).`, { stage, code: 'network' });
    }
    if (!response.ok) {
      // Do not include the upstream body: it can echo private cell values.
      throw new SheetsError(`${stage}: HTTP ${response.status}. ${HINTS[response.status] || 'Google could not complete the request.'}`, { status: response.status, stage });
    }
    try { return await response.json(); }
    catch { throw new SheetsError(`${stage}: Google returned an unreadable response.`, { stage, code: 'bad_response' }); }
  }
  ...
  return {
    ...
    async getSpreadsheet(id) {
      const data = await call('Open workbook', 'GET', `/${encodeURIComponent(id)}?${q({ fields: 'spreadsheetId,properties.title,sheets.properties(sheetId,title,index,gridProperties)' })}`);
    ...
    async batchGet(id, ranges) {
      ...
      const data = await call('Read workbook', 'GET', `/${encodeURIComponent(id)}/values:batchGet?${params}`);
      if (!Array.isArray(data.valueRanges)) throw new SheetsError('Read workbook: response did not contain the requested ranges.', { code: 'bad_response' });
      return data.valueRanges.map(vr => ({ range: vr.range, values: Array.isArray(vr.values) ? vr.values : [] }));
    },
    async update(id, range, values) {
      return call('Write workbook', 'PUT', `/${encodeURIComponent(id)}/values/${encodeURIComponent(range)}?${q({ valueInputOption: 'RAW' })}`, { range, majorDimension: 'ROWS', values });
    },
    async batchUpdateValues(id, data) {
      return call('Write workbook', 'POST', `/${encodeURIComponent(id)}/values:batchUpdate`, { valueInputOption: 'RAW', data: data.map(d => ({ range: d.range, majorDimension: 'ROWS', values: d.values })) });
    },
    async append(id, range, values) {
      return call('Append rows', 'POST', `/${encodeURIComponent(id)}/values/${encodeURIComponent(range)}:append?${q({ valueInputOption: 'RAW', insertDataOption: 'INSERT_ROWS' })}`, { range, majorDimension: 'ROWS', values });
    },
    async clear(id, range) {
      return call('Clear range', 'POST', `/${encodeURIComponent(id)}/values/${encodeURIComponent(range)}:clear`, {});
    },
    async batchUpdate(id, requests) {
      return call('Update workbook structure', 'POST', `/${encodeURIComponent(id)}:batchUpdate`, { requests });
    },
    async create(title, sheetTitles) {
      const data = await call('Create workbook', 'POST', '', {
        properties: { title },
