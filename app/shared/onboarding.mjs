...
export function validateInput(input) {
  if (JSON.stringify(input).length > SETUP_LIMITS.stagedChars) throw new Error(tr('This source is too large. Use a smaller export (maximum 1.5 million extracted characters).'));
  if (input.text && input.text.length > SETUP_LIMITS.textChars) throw new Error(tr('Document exceeds 80,000 extracted characters. Split it into smaller documents.'));
  if (!Array.isArray(input.tables) || input.tables.length > SETUP_LIMITS.tables) throw new Error(tr('Use at most 12 worksheets per source.'));
  const names = new Set();
  for (const table of input.tables) {
    if (!table.name || names.has(table.name)) throw new Error(tr('Each worksheet needs a different name.'));
    names.add(table.name);
    if (!Array.isArray(table.rows) || table.rows.length > SETUP_LIMITS.rows + 20) throw new Error(tr('{0}: maximum 5,000 data rows plus header rows.', table.name));
    for (const row of table.rows) {
      if (!Array.isArray(row) || row.length > SETUP_LIMITS.columns || row.some(v => v !== null && !['string', 'number', 'boolean'].includes(typeof v))) throw new Error(tr('{0}: unsupported or oversized row.', table.name));
    }
  }
  return input;
}
...
export function businessProfile(profile) {
  const business = { name: String(profile.name || '').trim(), model: profile.model || 'other', industry: profile.industry || '', description: profile.description || '', timezone: profile.timezone || 'Asia/Kuala_Lumpur', currency: String(profile.currency || 'MYR').toUpperCase(), locale: profile.locale || 'zh-CN', ai_guidance: profile.ai_guidance || '', synthetic: profile.synthetic === true };
  if (!business.name) throw new Error(tr('Enter your business name.'));
  try { new Intl.DateTimeFormat('en', { timeZone: business.timezone }); } catch { throw new Error(tr('Choose a valid timezone.')); }
  if (!/^[A-Z]{3}$/.test(business.currency)) throw new Error(tr('Currency must be a three-letter code, such as MYR.'));
  return business;
}
...
export function prepareSetup({ request, proposal, previous, schema, actor = 'owner' }) {
  ...
  if (!source || !/^[A-Za-z0-9_-]{1,40}$/.test(source.source_id)) throw new Error(tr('Invalid source.'));
  const input = validateInput(proposal.input);
  const selections = proposal.selections || [];
  if (!selections.length) throw new Error(tr('Select at least one supported table.'));
  ...
  for (const selection of selections) {
    if (!ENTITIES[selection.entity]) throw new Error(tr('Choose a supported table type.'));
    if (pkg.tables.some(t => t.entity === selection.entity)) throw new Error(tr('{0} already has an authoritative source. Edit that source instead of adding a duplicate.', selection.entity));
    const inputTable = input.tables.find(t => t.name === selection.name);
    if (!inputTable) throw new Error(tr('Missing worksheet: {0}', selection.name));
    ...
    if (!Number.isInteger(table.header_row) || table.header_row < 1 || table.header_row > inputTable.rows.length) throw new Error(tr('Choose the row containing column names.'));
    const headers = inputTable.rows[table.header_row - 1].map(v => String(v ?? '').trim().toLowerCase());
    for (const f of table.fields) if (headers.filter(h => h === f.header.trim().toLowerCase()).length !== 1) throw new Error(tr('Column "{0}" is missing or duplicated. Choose a unique column.', f.header));
    ...
    if (result.records.length > SETUP_LIMITS.rows) throw new Error(tr('Maximum 5,000 records per table.'));
  ...
  if (oldTables.some(t => !pkg.tables.some(n => n.table_id === t.table_id))) throw new Error(tr('Include every table already belonging to this source; removing tables is not supported by this update.'));
  ...
  if (source.kind === 'manual_package') {
    if (!isIsoDate(request.dataAsOf)) throw new Error(tr('Choose the date these file records describe.'));
...
export async function saveSetup(client, workspaceId, prepared, request, expectedSettings) {
  ...
  if (meta.lease_holder && Date.parse(meta.lease_expires) > Date.now()) throw new Error(tr('Import is running. Wait, then preview again.'));
  const { values: current } = await readKeyValues(client, workspaceId, 'Settings');
  if ((current.setup_package || '') !== (expectedSettings.setup_package || '') || Object.keys(prepared.entries).some(k => current[k] !== expectedSettings[k])) throw new Error(tr('This source changed after preview. Reload and review it again.'));
  if (parseJsonCell(current.setup_request)?.id !== request.id) throw new Error(tr('A newer source request exists. Reload before saving.'));
